...
class CartScreenController extends GetxController {
  ...
  Widget getLableText(text, {isMainTitle}) {
    return FadeInUp(
      child: Text(text,
          style: TextStyle(
            color: black,
            fontFamily: isMainTitle == true ? alegreya : null,
  ...
  double calculateFinalPrice(List<CommonProductList> cartItems) {
    ...
    for (CommonProductList item in cartItems) {
      logcat("finalProductPrice::1", item.toString());
      ...
      totalDeliveryPrice += double.parse(item.shippingCharge);
      totalDiscountPrice += double.parse(item.discount);

      // Calculate total item price
      double totalItemPrice = itemPrice - double.parse(item.discount);

      // Add shipping charge to the total item cost
      double totalCostForItem =
          totalItemPrice + double.parse(item.shippingCharge);

      // Add the total cost of the item to the grand total
      finalProductPrice.value += totalCostForItem;
      logcat("finalProductPrice::1", finalProductPrice.value.toString());
    ...
    logcat("finalProductPrice::", finalProductPrice.value.toString());
    update();
